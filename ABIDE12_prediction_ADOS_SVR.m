...
option.windowsize = 20;         % current TR parameter (In ABIDE, 10/20/30/40)
...
option.whichlambda = 2; % 2 = smallest lambda (i.e., least stringent)
...
if exist([Label_rawdata, '.mat'], 'file') == 2
    ...
    List_subj1 = BasicInfo1.subID;
    List_subj2 = BasicInfo2.subID;
    ...
    % Merge & Save =========================
    Sig_allsubj = vertcat(Sig_allsubj1, Sig_allsubj2);
    
    cd(Dirresult);
    eval(['save ',Label_data,' Sig_allsubj']);
...
if exist('Dfc_metrics', 'var') == 1
    fprintf('Load Def_metrics success \n');
...
Mat_fc_whole = Mat_fc_concat(Idx_pat,:); % only ASD group whose ADOS score
Mat_Dfc_whole = Mat_Dfc_concat(Idx_pat,:);
Mat_combine_whole = Mat_combine(Idx_pat,:);

% Extract A Specific Score (ADOS, ADI...)
TMP_Score = BasicInfo.ADOS_Total;   % ADOS_Total score
Array_Score = TMP_Score(Idx_pat,:);
clear TMP_Score*

Fol_save = sprintf('Elastic_%diter_based_n%d_%s_predict_ADOS',option.lassoiter,numel(Idx_pat),option.slidingwindow);
mkdir(Fol_save); cd(Fol_save);

%% ========== Machine learning Parts ==========
% SVR Parameter (Using LIBSVM for support vector regression)
cfg = [];
...
for nCV = 1:num_cv
    for nLasso = 1:option.lassoiter
        parfor nIter = 1:num_iter
            ...
            Idx_test = test(c,nCV);
            Score_train = Array_Score(Idx_train,:); 
            Score_test = Array_Score(Idx_test,:);
            ...
            [B_fc,FitInfo_fc] = lassoglm(Mat_fc_train,Score_train,'normal','CV',5,'NumLambda',200,'Options',statset('UseParallel',true),'Alpha',0.5); 
            [B_Dfc,FitInfo_Dfc] = lassoglm(Mat_Dfc_train,Score_train,'normal','CV',5,'NumLambda',200,'Options',statset('UseParallel',true),'Alpha',0.5);
            [B_combine,FitInfo_combine] = lassoglm(Mat_combine_train,Score_train,'normal','CV',5,'NumLambda',200,'Options',statset('UseParallel',true),'Alpha',0.5); % ,'Alpha',0.5
...
for nCV = 1:num_cv
    ...
    %% Load Feature selection results (LASSO regression or Elastic-net)
    Mat_fc_train = Mat_fc_whole(Idx_train,:);
    Mat_Dfc_train = Mat_Dfc_whole(Idx_train,:);
    Mat_combine_train = Mat_combine_whole(Idx_train,:);
    ...
    parfor ifeature = 1:n_feature  
        ...
        if numel(Predictor_fc) > 0
            model_fc = train_svm(cfg, i_Mat_fc_train{ifeature}, Score_train);
            [Pred_fc] = test_svm(model_fc, i_Mat_fc_test{ifeature});
            MSE_fc = mv_calculate_performance('mean_squared_error', [], Pred_fc, Score_test);
            w_fc = (model_fc.model.SVs' * model_fc.model.sv_coef)'; 
            b_fc = -model_fc.model.rho;
        ...
        if numel(Predictor_Dfc) > 0
            model_Dfc = train_svm(cfg, i_Mat_Dfc_train{ifeature}, Score_train);
            [Pred_Dfc] = test_svm(model_Dfc, i_Mat_Dfc_test{ifeature});
            MSE_Dfc = mv_calculate_performance('mean_squared_error', [], Pred_Dfc, Score_test);
            w_Dfc = (model_Dfc.model.SVs' * model_Dfc.model.sv_coef)';
            b_Dfc = -model_Dfc.model.rho; 
        elseif numel(Predictor_Dfc) == 0
            MSE_Dfc = 0;
        end

        if numel(Predictor_combine) > 0
            model_combine = train_svm(cfg, i_Mat_combine_train{ifeature}, Score_train);
            [Pred_combine] = test_svm(model_combine, i_Mat_combine_test{ifeature});
            MSE_combine = mv_calculate_performance('mean_squared_error', [], Pred_combine, Score_test);
            w_combine = (model_combine.model.SVs' * model_combine.model.sv_coef)';
            b_combine = -model_combine.model.rho; 
        ...
        % Save
        TMP_MSE_fc{1,ifeature}(nCV,nIter) = MSE_fc; 
        TMP_MSE_Dfc{1,ifeature}(nCV,nIter) = MSE_Dfc; 
        TMP_MSE_combine{1,ifeature}(nCV,nIter) = MSE_combine; 

        Cell_svr_w_fc{nCV,ifeature,nIter} = w_fc;
        Cell_svr_w_Dfc{nCV,ifeature,nIter} = w_Dfc;
        Cell_svr_w_combine{nCV,ifeature,nIter} = w_combine;
        Cell_svr_b_fc{nCV,ifeature,nIter} = b_fc;
        Cell_svr_b_Dfc{nCV,ifeature,nIter} = b_Dfc;
        Cell_svr_b_combine{nCV,ifeature,nIter} = b_combine;
...
if option.whichlambda == 1
    Dir_name = sprintf('SVR_setlambda_lasso_n%d_%s_%diter',numel(Idx_pat),option.slidingwindow,num_iter);
elseif option.whichlambda == 2
    Dir_name = sprintf('SVR_feature%d_lasso_n%d_%s_%diter',n_feature,numel(Idx_pat),option.slidingwindow,num_iter);
end
mkdir(Dir_name); cd(Dir_name);

for ifeature = 1:n_feature
    Cell_MSE_fc{1,ifeature} = mean(TMP_MSE_fc{1,ifeature},1)'; 
    Cell_MSE_Dfc{1,ifeature} = mean(TMP_MSE_Dfc{1,ifeature},1)'; 
    Cell_MSE_combine{1,ifeature} = mean(TMP_MSE_combine{1,ifeature},1)'; 
...
if option.whichlambda == 1
    ...
    for nIter = 1:num_iter  % Cell_svm_w_fc{nCV,ifeature,nIter}
        for nCV = 1:num_cv
            Cur_weight_fc = Cell_svr_w_fc{nCV,1,nIter};
            Cur_weight_Dfc = Cell_svr_w_Dfc{nCV,1,nIter};
            Cur_weight_combine = Cell_svr_w_combine{nCV,1,nIter};
            
            % Case: fc (static-fc)
            if numel(Cur_weight_fc) == 0 % when no feature was survived
                Mat_svr_w_fc(:,:,nCV,nIter) = Template_weights1;
            elseif numel(Cur_weight_fc) > 0
                tmp_svr_w_fc = Template_weights1;
                for nFeature = 1:length(Cur_weight_fc), tmp_svr_w_fc(Idxref_Predictor_fc{nCV,nIter}(nFeature)) = Cur_weight_fc(nFeature); end
                Mat_svr_w_fc(:,:,nCV,nIter) = tmp_svr_w_fc; clear tmp_svr_w_fc
            end

            % Case: Dfc (MSSD)
            if numel(Cur_weight_Dfc) == 0 % when no feature was survived
                Mat_svr_w_Dfc(:,:,nCV,nIter) = Template_weights1;
            elseif numel(Cur_weight_Dfc) > 0
                tmp_svr_w_Dfc = Template_weights1;
                for nFeature = 1:length(Cur_weight_Dfc), tmp_svr_w_Dfc(Idxref_Predictor_Dfc{nCV,nIter}(nFeature)) = Cur_weight_Dfc(nFeature); end
                Mat_svr_w_Dfc(:,:,nCV,nIter) = tmp_svr_w_Dfc; clear tmp_svr_w_Dfc
            end
            
            % Case: combine
            if numel(Cur_weight_combine) == 0 % when no feature was survived
                Mat_svr_w_combine(:,:,nCV,nIter) = Template_weights2;
            elseif numel(Cur_weight_combine) > 0
                tmp_svr_w_combine = Template_weights2;
                for nFeature = 1:length(Cur_weight_combine), tmp_svr_w_combine(Idxref_Predictor_combine{nCV,nIter}(nFeature)) = Cur_weight_combine(nFeature); end
                Mat_svr_w_combine(:,:,nCV,nIter) = tmp_svr_w_combine;
            end
    ...
    Plot_svr_w_fc = mean(Mat_svr_w_fc,4); Plot_svr_w_fc = mean(Plot_svr_w_fc,3);
    Plot_svr_w_Dfc = mean(Mat_svr_w_Dfc,4); Plot_svr_w_Dfc = mean(Plot_svr_w_Dfc,3);
    Plot_svr_w_combine = mean(Mat_svr_w_combine,4); Plot_svr_w_combine = mean(Plot_svr_w_combine,3);
...
elseif option.whichlambda == 2    
    ...
    %% Wave graph
    % Plotting wave graph
    shadedErrorBar(1:n_feature,Mean_MSE_fc,Std_MSE_fc,{'k','markerfacecolor','r'}); hold on
    shadedErrorBar(1:n_feature,Mean_MSE_Dfc,Std_MSE_Dfc,{'b','markerfacecolor','r'}); hold on
    shadedErrorBar(1:n_feature,Mean_MSE_combine,Std_MSE_combine,{'g','markerfacecolor','r'}); 
